...
The goal is to port this skill into the ben-cp repo so it:
- Lives alongside other product skills (under `skills/product/` once that consolidation is complete)
- Is versioned, auditable, and improvable through the standard repo workflow
- Can be referenced and updated by meta-agents (Changelog Auditor, Repo Auditor, etc.)
- Becomes the source of truth — the Cowork plugin version should eventually be replaced by a symlink or thin wrapper pointing back to the repo copy
...
1. **Load context** — Read `AGENTS.md`, `skills/product/index.md` (if it exists), repo root `index.md`
2. **Decide placement** — See Task 2
3. **Create skill directory** — Migrate all content with repo-native formatting
4. **Update repo root `index.md`** — Register the new skill
...
- Repo root `index.md` — understand where skills currently live
...
- Create `skills/task-capture/` at the repo skill root
...
**Formatting notes:**
- `index.md` should follow the repo's standard skill index format (see any existing `skills/*/index.md` for the convention)
- `procedure.md` is the main content — adapt the SKILL.md content into repo-native prose/steps rather than keeping the Cowork agent-instruction framing. The repo version is documentation; the Cowork version is a runtime prompt.
...
After the repo version exists, update the Cowork plugin `SKILL.md` to add a note at the top:

```
> **Source of truth:** This skill is mirrored from the ben-cp repo at
> `skills/[product/]task-capture/procedure.md`. Update the repo version first;
> keep this file in sync manually until a symlink or auto-sync is in place.
```

Do not delete or replace the Cowork plugin version — it must remain functional for runtime use. The repo version is the canonical reference and edit target.

---

## Task 5: Update Registry Files

- Repo root `index.md`: add `task-capture` entry (or `product/task-capture` if under product/)
...
- The Cowork plugin SKILL.md is written as an agent-instruction prompt (imperative, "you are..."). The repo `procedure.md` should be written as documentation — the same information but in third-person reference format. Both are valid, just for different audiences.
