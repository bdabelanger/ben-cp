...
#
# Loads credentials from .env at vault root, then runs the pipeline.
# Logs are appended to outputs/status-reports/logs/run.log.
#
# Crontab example (every Monday at 8am):
#   0 8 * * 1 /path/to/ben-cp/tools/status-reports/run_pipeline.sh --force >> /dev/null 2>&1

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VAULT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"   # orchestration/pipelines/product/projects → vault root
ENV_FILE="$VAULT_ROOT/.env"
LOG_FILE="$SCRIPT_DIR/outputs/status-reports/logs/run.log"

mkdir -p "$SCRIPT_DIR/outputs/status-reports/logs"
...
python3 "$SCRIPT_DIR/pipeline/full_run.py" "$@" 2>&1 | tee -a "$LOG_FILE"
