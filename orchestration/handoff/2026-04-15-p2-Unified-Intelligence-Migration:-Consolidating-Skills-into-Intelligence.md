...
- **New Home**: `intelligence/core/skills/`
- **System Domain**: Establish `intelligence/core/` for all system-level logic (policies, agents, skills).
...
- **MCP Server**: Update `src/ben-cp.ts` constants (`skillsPath`) to point to the new `intelligence/core/skills/` location.
...
- **Separation Policy**: Move `skills/orchestration/separation-policy.md` to `intelligence/core/governance/policy.md` and update its definitions.
...
- **Mkdir**: Create the new core hierarchy: `intelligence/core/{skills,governance,mapping}`.
- **Move**: `git mv skills/* intelligence/core/skills/`.
...
- [ ] `get_skill(relativePath: 'orchestration/handoff/SKILL.md')` resolves correctly to `intelligence/core/skills/orchestration/handoff/SKILL.md`.
