...
`agents/` is being introduced as a repo root directory to house agent-produced artifacts — things agents make, log, or leave behind that aren't intelligence records, skills, or handoffs. `agents/art/` is the first subdirectory (see separate handoff). This handoff establishes the full `agents/` structure including logs and session notes.

## Goal

Define and scaffold `agents/` as a first-class repo directory with subdirectories for art, logs, and session notes.
...
3. Update `AGENTS.md` repo tree to include `agents/` at root level
...
5. Update repo separation policy (`intelligence/governance/policy.md`) to document `agents/` as a first-class directory

## Verification

- `agents/`, `agents/logs/`, `agents/sessions/` all exist with `index.md` files
- `AGENTS.md` repo tree reflects the new directory
