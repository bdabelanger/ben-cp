...
When this graduates to MCP-level, the natural shape is a session-scoped key/value store: `quartermaster.set(key, value)` / `quartermaster.get(key)` / `quartermaster.close()` (which auto-writes the changelog entry and self-destructs). The current file-based convention maps cleanly onto that future interface — worth designing the template with that migration in mind so the fields translate directly.
...
**On naming:**
The session planning artifact is named `notes.md` (renamed from `quartermaster.md`). The skill itself retains the name "quartermaster." Character names are not used in skill references.

---

## Execution Order

1. **Verify Infrastructure** — Check `skills/pmm/index.md` and `AGENTS.md` for the new rules.
2. **Rename artifact** — Update `pmm/index.md` and `pmm/report.md` to reference `notes.md` instead of `quartermaster.md` throughout.
...
- Strategic PM is intended to eventually become an MCP-level tool. Design the template fields with that future interface in mind (`set` / `get` / `close`).
- Always delete the `notes.md` file *after* the changelog is written but *before* ending the session.
- Character names are not used in skill references — use the generic skill name only.
