...
During the 2026-04-25 session, Cowork (Claude) needed to read the nightly dream report output (`orchestration/pipelines/outputs/dream/daily-report.md`) in order to embed it into a Gemma smoke test handoff.
...
The `get_intelligence` tool is scoped to `intelligence/` only. The `get_handoff` tool is scoped to `orchestration/handoff/` only. There is no ben-cp MCP tool that can read files from `orchestration/pipelines/outputs/`.
...
Add a purpose-built MCP tool — or extend `get_intelligence` — to allow reading files from `orchestration/pipelines/outputs/`. Suggested interface:
```
get_report(scope='dream')         → reads orchestration/pipelines/outputs/dream/daily-report.md
...
Add a note in the ben-cp server README or AGENTS.md that direct filesystem reads of `orchestration/pipelines/outputs/` are unreliable on Google Drive due to sync latency. Agents should always use MCP tools (once Fix 2 is in place) rather than raw filesystem access for pipeline outputs.
