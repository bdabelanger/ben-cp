...
# orchestration/pipelines/product/projects/pipeline/ -> ben-cp/
...
def reset_manifest():
    ...
    for dir_to_archive in [proc_dir, os.path.join(VAULT_ROOT, "outputs/status-reports")]:
        if os.path.exists(dir_to_archive):
            for filename in os.listdir(dir_to_archive):
                if filename.endswith(('.json', '.md', '.html')):
                    ...
                    # Reports get a clean dated name; everything else gets the archived_ prefix
                    if filename == "Status_Report.html":
                        dst = os.path.join(arch_dir, f"Status_Report_{date_str}.html")
                    else:
    ...
    for step in data['steps']:
        ...
        if step['id'] == "1_asana_ingest":
            step['file'] = "inputs/status-reports/processed/asana_active.json"
        elif step['id'] == "2_atlassian_fetch":
            step['file'] = "inputs/status-reports/raw/jira"
        elif step['id'] == "3_rovo_context":
            step['file'] = "inputs/status-reports/processed/rovo_insights.json"
        elif step['id'] == "4_jira_harvest":
            step['file'] = "inputs/status-reports/processed/jira_issues.json"
        elif step['id'] == "5_report_generation":
            step['file'] = "outputs/status-reports/Status_Report.html"
