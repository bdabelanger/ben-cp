...
# Context
The Dream cycle context sensor (2026-04-26) flagged several files as token economy risks due to size or accumulation. The user prefers to handle these via vault handoffs rather than Asana tasks.

# Risks Identified

## RED FLAGS (>750KB - action required)
- `intelligence/product/shareout/q2/source/Q2 2026 Product Shareout.pdf` — 7,410 KB (7.2 MB)
  - **Question**: Should this large PDF live in the vault or be archived/offloaded?
...
1. **Q2 Shareout PDF**: Large binary PDF moved to `source/archive/`.
   - **Decision**: Keep in archive for Drive reference, but exclude from vault health scans (which ignore `archive/` paths).
2. **Raw Data Purge**: 
