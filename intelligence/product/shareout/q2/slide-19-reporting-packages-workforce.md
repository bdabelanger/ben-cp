---
title: Slide 19 Workforce Reporting Package
type: intelligence
domain: intelligence/product/shareout/q2
---

# Slide 19: Workforce Reporting Package

- **okr:** Simplify Reporting
- **slide:** 19
- **status:** needs_work

## Slide 19 — Reporting packages: Understand Work and Workflow Performance

- **Theme:** Simplify Reporting
- **Status:** ⚠️ NEEDS WORK
- **Release Quarter:** Q2 2026

## Content Summary
- **Primary Goal:** Make workload and task ownership visible at a glance.
- **Capabilities:**
    - Visibility into individual and team manageability.
    - Analysis of work type complexity and time allocation.
    - Identification of process bottlenecks or compliance gaps.

## 🗣️ Key Narrative Points
- **Headline:** Reporting packages: Understand Work and Workflow Performance
- **Quote Placeholder:** *"A customer said this."* (NEEDS REAL QUOTE)

## Feedback & Internal Notes
- **Missing:** Needs a real customer quote to replace the current placeholder.

## Constituent Projects
- **Reporting - Tasks/Work package**

