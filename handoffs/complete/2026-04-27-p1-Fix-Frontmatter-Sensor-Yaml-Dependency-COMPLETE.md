...
This sensor has been blind since at least this run — it cannot audit any frontmatter in the vault while the `yaml` (PyYAML) dependency is missing from the environment.
...
1. Locate the dream sensor runner — likely `reports/dream/run.py` or similar in the vault's `src/` directory.
