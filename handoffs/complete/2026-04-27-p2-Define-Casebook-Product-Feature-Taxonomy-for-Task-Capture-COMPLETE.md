...
Ben provided the canonical product and feature lists. Cowork created intelligence/product/taxonomy.md with the full product list, feature list, keyword inference map, format convention, and label examples. Step 2 complete. Step 3 (Code: wire into capture_task) is ready to pick up.
...
- Casebook has a product/feature hierarchy (e.g. Engage → Notes, Enrollments, WLV) that isn't formally documented in the repo
- The inference map in `capture_task` should reference a repo intelligence record, not hardcoded guesses
