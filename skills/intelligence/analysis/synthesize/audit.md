---
title: Audit Procedure Synthesize
type: intelligence
domain: skills/intelligence/analysis/synthesize
---

# Audit Procedure: Synthesize

> **Owner:** Intelligence (Synthesize)

## Requirements
- [ ] Non-linear observation: Identify links between disparate domains.
- [ ] Essential narrative: Look beyond raw status to find the project's pulse.
- [ ] Target exactly one or two primary "technical truths."

## Operating Procedure

### Step 1: Saturation
1. Read all `vault/brain/` session summaries.
2. Review recent root changelogs for relevant context.
3. Read `Intelligence (Predict)` output for forward-looking signals.

### Step 2: Connection Identification
Identify connections that aren't stated explicitly. If a backend refactor (Platform) accidentally improved a search feature (Product), that is a synthesis learning.

### Step 3: Distillation
Synthesize findings into a single, high-density insight.
- **Why this truth?**
- **What is the signal strength?**
- **How does it change the overall project trajectory?**
